...
public class ForgotPasswordAtion {
    ...
    public boolean checkMessageShow(WebDriver driver,String message){
        boolean result = false;
        try {
            boolean messageWhenPass = driver.findElements(By.xpath("/html/body/div[3]/div/div/h1")).size()>0;
            // khi chạy case thành công
            if (messageWhenPass){
                String messageResult = driver.findElement(By.xpath("/html/body/div[3]/div/div/h1")).getText();
                if (messageResult.contains(message)){
                    result = true;
                }
            }else { // khi chạy case thất bại
                String messageResult = driver.findElement(By.xpath("/html/body/div[3]/div/div/div[1]")).getText();
                if (messageResult.contains(message)){
                    result = true;
                }
            }
