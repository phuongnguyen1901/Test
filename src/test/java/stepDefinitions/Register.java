...
public class Register {
    ...
    public void checkMessageInEmail(){
        try{
            ...
            Setup.testLogs.log(LogStatus.PASS,"Hiển thị thông báo lỗi : Email này đã được đăng kí. Vui lòng nhập email khác thành công","");
            Setup.pass++;
            setup.after(true);
        }catch (AssertionError e){
    ...
    public void checkMessageInPassword(){
        try{
            assertTrue(registerAction.checkResult(driver,dataItem.get("Message").toString(), By.id("invalid_password_feedback")));
